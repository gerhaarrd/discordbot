...
class ReputationSystem:
    ...
    def get_top_users(self, limit: int = 10) -> List[Tuple[int, int]]:
        """Retorna lista dos top usuários sempre a partir do banco"""
        users_rep = database.get_top_users(limit)
        return [(int(uid), rep) for uid, rep in users_rep]
...
async def register(bot):
    ...
    async def reptop_command(interaction: discord.Interaction):
        ...
        try:
            ...
            # A interação expirou antes do ack (token inválido no Discord).
            return
        except Exception:
            return
        ...
        total_users = len(database.get_all_users_rep())
        embed.set_footer(text=f"Total de {total_users} usuários registrados")
        embed.timestamp = discord.utils.utcnow()
